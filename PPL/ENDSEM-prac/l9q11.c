...
//__global__ void add(int *A,int *B,int *C) q13
    int i = threadIdx.x;
    for(int j=0;j<c;j++)
        C[i*r+j] = A[i*r+j] + B[i*r+j];
    // C[i+j*r] = A[i+j*r] + B[i+j*r]; for q12
//C[i] = A[i] + B[i]; for q13 no for loop
}
...
int main(){
    ...
    //add<<<1,c2>>>(dA,dB,dC,r1,c2); for q12
    //add<<<1,r1*c2>>>(dA,dB,dC); for q13
